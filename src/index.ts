...
async function loadCredentials() {
  try {
    ...
    const callback =
      process.argv[2] === "auth" && process.argv[3] ? process.argv[3] : "http://localhost:3000/oauth2callback";
...
const ReadEmailSchema = z.object({
  query: z.string().describe("Search query for the email (e.g., 'newer_than:1d')"),
});
...
async function main() {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: "read_email",
        description: "Search the latest email matching query and read its content",
    ...
    try {
      switch (name) {
        ...
        case "read_email": {
          ...
          console.log("🔵 [gmail_read_email] called with query:", validatedArgs.query);
          ...
          if (!messages || messages.length === 0) {
            throw new Error("해당 조건에 맞는 메일을 찾을 수 없습니다.");
          }
          ...
          // 안전한 본문 추출
          const extractEmailContent = (part: GmailMessagePart): { text: string; html: string } => {
            let text = "";
            let html = "";

            const walk = (p: GmailMessagePart | undefined) => {
              if (!p) return;
          ...
          if (!payload) throw new Error("이메일 payload가 없습니다");
          const { text, html } = extractEmailContent(payload as GmailMessagePart);
          let body = text || html || "(본문 없음)";
          if (!text && html) {
            body = "[HTML 메일입니다]\n\n" + html;
          }
        ...
        case "get_or_create_label": {
          ...
          const action =
            result.type === "user" && result.name === validatedArgs.name ? "found existing" : "created new";
